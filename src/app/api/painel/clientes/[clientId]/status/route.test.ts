...
describe("api/painel/clientes/[clientId]/status", () => {
  beforeEach(() => {
  ...
  it("encaminha o PATCH para o wrapper de rota do painel", async () => {
    ...
    const { PATCH } = await import(
      "@/app/api/painel/clientes/[clientId]/status/route"
    );

    const request = new Request("https://example.test/api/painel/clientes/248/status", {
      method: "PATCH",
